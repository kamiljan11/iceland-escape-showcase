    ...
    <section className="relative min-h-[85vh] md:min-h-screen flex items-end md:items-center overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 md:px-6 pb-16 md:pb-0">
        <div className="max-w-2xl">
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="font-heading text-3xl md:text-6xl lg:text-7xl font-bold leading-[1.1] mb-4 md:mb-6 text-white">
            {t({
              en: "We show you the Iceland",
              pl: "Pokazujemy Islandię,",
              is: "Við sýnum þér Íslandið",
            }, lang)}
            <span className="block text-[hsl(160,60%,65%)]">
              {t({
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="text-white/80 text-sm md:text-lg max-w-lg mb-8 md:mb-10 font-body leading-relaxed">
            {t({
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.35 }} className="flex flex-col sm:flex-row gap-3 md:gap-4">
            <a href="#tours" className="inline-flex items-center justify-center gap-2 px-6 md:px-8 py-3.5 md:py-4 rounded-lg bg-primary font-body font-semibold text-sm text-primary-foreground hover:opacity-90 transition-opacity">
              <MapPin className="w-4 md:w-5 h-4 md:h-5" />
              {t(tr.hero.cta_tours, lang)}
            </a>
            <a href="#story" className="inline-flex items-center justify-center gap-2 px-6 md:px-8 py-3.5 md:py-4 rounded-lg bg-white/20 backdrop-blur-sm font-body font-semibold text-sm text-white hover:bg-white/30 transition-colors">
